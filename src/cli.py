...
@click.option(
    "--output-dir", "-o", type=click.Path(), default="results", help="Output directory"
)
...
def cluster(ctx, method, data_path, output_dir, n_clusters):
    ...
    # Set random seed
    set_random_seed(
        config.model.random_state if hasattr(config.model, "random_state") else 42
    )
...
@cli.command()
@click.option(
    "--n-samples", "-n", type=int, default=100, help="Number of samples to generate"
)
@click.option("--n-clusters", "-k", type=int, default=3, help="Number of clusters")
@click.option(
    "--output-path",
