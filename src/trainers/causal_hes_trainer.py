...
class CausalHESTrainer:
    ...
    def train(
        ...
        self.logger.info("=" * 80)
        self.logger.info(
            f"Data shapes - Load: {load_data.shape}, Weather: {weather_data.shape}"
        )
        self.logger.info(
            f"Training strategy - CSSAE epochs: {cssae_epochs}, Joint epochs: {joint_epochs}"
        )
        ...
        if evaluate_separation:
            ...
            separation_results = self._evaluate_source_separation(
                load_tensor, weather_tensor
            )
        ...
        if evaluate_clustering:
            ...
            clustering_results = self._evaluate_clustering(
                load_tensor, weather_tensor, true_labels
            )
    ...
    def _evaluate_source_separation(
        self, load_data: torch.Tensor, weather_data: torch.Tensor
    ) -> Dict[str, float]:
        """Evaluate source separation quality."""
        # Get source separation results
        separation_results = self.model.get_source_separation_results(
    ...
    def _evaluate_clustering(
        ...
        if true_labels is not None:
            ...
            base_embeddings = self.model.cssae.get_base_embeddings(
                load_data, weather_data
            )
            ...
            metrics = {
                "silhouette_score": silhouette_score(
                    base_embeddings_np, predicted_labels
                ),
    ...
    def _print_clustering_report(self, clustering_results: Dict):
        ...
        print(
            f"Calinski-Harabasz:       {clustering_results['calinski_harabasz_score']:.4f}"
        )
        print(
            f"Davies-Bouldin:          {clustering_results['davies_bouldin_score']:.4f}"
        )
    ...
    def get_training_summary(self) -> Dict:
        ...
        summary = {
            ...
            "training_completed": {
                "cssae_pretraining": self.training_history["cssae_pretraining"]
                is not None,
        ...
        if self.training_history["evaluation_metrics"]:
            ...
            if "source_separation" in eval_metrics:
                ...
                summary["source_separation_quality"] = eval_metrics[
                    "source_separation"
                ].get("independence_independence_score", 0.0)
