...
class CSSAETrainer:
    ...
    def __init__(
        ...
        self.logger.info(
            f"Loss weights - Reconstruction: {reconstruction_weight}, Causal: {causal_weight}"
        )
        self.logger.info(
            f"Curriculum learning: {curriculum_learning}, Warmup epochs: {warmup_epochs}"
        )
    ...
    def _compute_losses(
        ...
        recon_loss = self.reconstruction_loss(
            load_data, outputs["total_reconstruction"]
        )
    ...
    def evaluate_separation_quality(
        self, load_data: np.ndarray, weather_data: np.ndarray, sample_size: int = 1000
    ) -> Dict[str, float]:
        """
    ...
    def plot_training_history(self, save_path: Optional[str] = None):
        ...
        axes[0, 1].plot(
            self.history["epoch"], self.history["causal_loss"], label="Causal"
        )
        ...
        # Loss ratio
        if (
            len(self.history["reconstruction_loss"]) > 0
            and len(self.history["causal_loss"]) > 0
        ):
            loss_ratio = np.array(self.history["causal_loss"]) / (
                np.array(self.history["reconstruction_loss"]) + 1e-8
            )
