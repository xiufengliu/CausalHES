...
class StatisticalTester(LoggerMixin):
    ...
    def test_clustering_significance(
        ...
        results["random_clustering_test"] = self._test_against_random_clustering(
            X, labels
        )
    ...
    def compare_clustering_methods(
        self, X: np.ndarray, labels_list: List[np.ndarray], method_names: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        for i in range(len(method_names)):
            for j in range(i + 1, len(method_names)):
                ...
                results["pairwise_comparisons"][comparison_key] = (
                    self._compare_two_methods(X, labels1, labels2, method1, method2)
                )
    ...
    def _test_against_random_clustering(
        self, X: np.ndarray, labels: np.ndarray
    ) -> Dict[str, Any]:
        """
        ...
        effect_size = (actual_silhouette - np.mean(random_silhouettes)) / np.std(
            random_silhouettes
        )
    ...
    def _bootstrap_silhouette_score(
        self, X: np.ndarray, labels: np.ndarray
    ) -> Dict[str, Any]:
        """
    ...
    def _compute_hopkins_statistic(
        ...
        random_points = np.random.uniform(
            min_vals, max_vals, size=(n_samples, X.shape[1])
        )
    ...
    def _friedman_test(
        ...
        try:
            ...
            ranking = sorted(
                zip(method_names, avg_scores), key=lambda x: x[1], reverse=True
            )
