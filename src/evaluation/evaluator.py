...
class ClusteringEvaluator(LoggerMixin):
    ...
    def __init__(
        ...
        self.statistical_tester = (
            StatisticalTester() if include_statistical_tests else None
        )
    ...
    def evaluate(
        ...
        if self.include_statistical_tests and self.statistical_tester is not None:
            ...
            try:
                statistical_results = (
                    self.statistical_tester.test_clustering_significance(X, labels)
                )
    ...
    def compare_methods(
        ...
        if primary_metric in ["davies_bouldin_score", "inertia"]:
            ...
            comparison_df = comparison_df.sort_values(
                f"internal_{primary_metric}", ascending=True
            )
        else:
            # Higher is better
            comparison_df = comparison_df.sort_values(
                f"internal_{primary_metric}", ascending=False
            )
    ...
    def _evaluate_uncertainty(self, probabilities: np.ndarray) -> Dict[str, Any]:
        ...
        lower_bounds, upper_bounds = (
            self.uncertainty_metrics.compute_confidence_intervals(probabilities)
        )
    ...
    def generate_report(
        self, method_name: str, output_path: Optional[Union[str, Path]] = None
    ) -> str:
        """
        ...
        # Internal metrics
        if result["internal_metrics"]:
            report_lines.extend(["Internal Validation Metrics:", "-" * 30])
            for metric, value in result["internal_metrics"].items():
        ...
        # External metrics
        if result["external_metrics"]:
            report_lines.extend(["External Validation Metrics:", "-" * 30])
            for metric, value in result["external_metrics"].items():
                if not np.isnan(value):
                    report_lines.append(f"  {metric}: {value:.4f}")
                else:
        ...
        # Uncertainty metrics
        if result["uncertainty_metrics"]:
            report_lines.extend(["Uncertainty Metrics:", "-" * 20])
            for metric, value in result["uncertainty_metrics"].items():
                if not np.isnan(value):
                    report_lines.append(f"  {metric}: {value:.4f}")
                else:
        ...
        # Stability metrics
        if result["stability_metrics"]:
            report_lines.extend(["Stability Metrics:", "-" * 18])
            for metric, value in result["stability_metrics"].items():
                if not np.isnan(value):
                    report_lines.append(f"  {metric}: {value:.4f}")
                else:
        ...
        if result["statistical_tests"]:
            report_lines.extend(["Statistical Significance Tests:", "-" * 32])
    ...
    def get_best_method(
        self, metric: str = "silhouette_score", higher_is_better: bool = True
    ) -> Tuple[str, float]:
        """
        ...
        for method_name, result in self.results.items():
            ...
            if result["internal_metrics"] and metric in result["internal_metrics"]:
                ...
                and metric in result["uncertainty_metrics"]
            ):
