...
def calculate_independence_score(base_embedding: np.ndarray, 
                               weather_embedding: np.ndarray) -> Dict[str, float]:
    """
...
def calculate_reconstruction_metrics(original: np.ndarray,
...
def calculate_source_separation_quality(original_load: np.ndarray,
                                      base_load: np.ndarray,
                                      weather_effect: np.ndarray,
                                      reconstructed_total: np.ndarray) -> Dict[str, float]:
    """
    ...
    total_recon_metrics = calculate_reconstruction_metrics(original_load, reconstructed_total)
    ...
    return {
        ...
        'separation_quality_score': total_recon_metrics['r2'] * (1.0 / (1.0 + additivity_error))
    }


def calculate_source_separation_metrics(original_load: np.ndarray,
                                      base_embedding: np.ndarray,
                                      weather_embedding: np.ndarray,
                                      reconstructed_total: np.ndarray,
                                      reconstructed_base: np.ndarray,
                                      reconstructed_weather_effect: np.ndarray) -> Dict[str, float]:
    """
    ...
    independence_metrics = calculate_independence_score(base_embedding, weather_embedding)
    
    # 2. Reconstruction metrics
    reconstruction_metrics = calculate_reconstruction_metrics(original_load, reconstructed_total)
    
    # 3. Source separation quality
    separation_metrics = calculate_source_separation_quality(
        original_load, reconstructed_base, reconstructed_weather_effect, reconstructed_total
    )
    
    # 4. Component-specific reconstruction metrics
    base_recon_metrics = calculate_reconstruction_metrics(original_load, reconstructed_base)
    ...
    logger.info(f"Source separation evaluation completed. Overall score: {overall_score:.4f}")
...
def print_source_separation_report(metrics: Dict[str, float]):
    ...
    print(f"Distance Correlation:    {metrics['independence_distance_correlation']:.6f}")
    print(f"Pearson Correlation:     {metrics['independence_pearson_correlation']:.6f}")
    print(f"Independence Score:      {metrics['independence_independence_score']:.6f} ⭐")
    ...
    print(f"Weather Responsiveness: {metrics['separation_weather_responsiveness_ratio']:.6f}")
    print(f"Separation Score:       {metrics['separation_separation_quality_score']:.6f} ⭐")
