...
class ClusterVisualizer:
    ...
    def plot_cluster_profiles(
        ...
        for i, label in enumerate(unique_labels):
            ...
            for j in range(min(20, len(cluster_data))):  # Show max 20 profiles
                axes[i].plot(
                    cluster_data[j, :, 0], alpha=0.3, color="gray", linewidth=0.5
                )
    ...
    def plot_source_separation(
        ...
        for i in range(n_examples):
            ...
            # Weather component
            axes[i, 2].plot(
                weather_component[i, :, 0], "r-", linewidth=2, label="Weather"
            )
            ...
            axes[i, 3].plot(
                reconstructed_load[i, :, 0], "r--", linewidth=2, label="Reconstructed"
            )
    ...
    def plot_performance_comparison(
        ...
        for i, metric in enumerate(metrics):
            ...
            for method, results in results_dict.items():
                if (
                    "clustering_metrics" in results
                    and metric in results["clustering_metrics"]
                ):
    ...
    def plot_attention_gates(
        ...
        if weather_data.shape[1] > 1:
            ax2_twin = ax2.twinx()
            ax2_twin.plot(
                hours, weather_data[:, 1], "g-", linewidth=2, label="Humidity"
            )
