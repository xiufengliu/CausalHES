...
def create_time_windows(data: np.ndarray,
                       window_size: int,
                       step_size: int = 1,
    ...
    if window_size > n_timesteps:
        raise ValueError(f"Window size ({window_size}) cannot be larger than timesteps ({n_timesteps})")
...
def validate_data_shapes(load_data: np.ndarray, 
    ...
    if load_data.shape[0] != weather_data.shape[0]:
        raise ValueError(f"Load and weather data must have same number of samples: "
                        f"{load_data.shape[0]} vs {weather_data.shape[0]}")
    
    if load_data.shape[1] != weather_data.shape[1]:
        raise ValueError(f"Load and weather data must have same number of timesteps: "
                        f"{load_data.shape[1]} vs {weather_data.shape[1]}")
    
    if labels is not None and len(labels) != load_data.shape[0]:
        raise ValueError(f"Labels must have same length as data: "
...
def interpolate_missing_values(data: np.ndarray,
                             method: str = 'linear',
...
def _interpolate_1d(series: np.ndarray,
                   method: str,
...
def split_data(data: Union[np.ndarray, List[np.ndarray]],
...
def check_data_quality(data: np.ndarray,
                      name: str = "data") -> Dict[str, Union[int, float, bool]]:
    """
