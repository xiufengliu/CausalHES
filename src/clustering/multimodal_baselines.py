...
class ContrastiveMVClustering(BaseMultiModalClusteringMethod, LoggerMixin):
    ...
    def fit(
        ...
        if X_secondary is None:
            raise ValueError(
                "Contrastive MV clustering requires secondary data (weather)"
            )

        self.log_info(
            f"Training contrastive multi-view clustering for {epochs} epochs..."
        )
        ...
        self.weather_encoder_ = ViewEncoder(weather_dim, self.embedding_dim).to(
            self.device
        )

        optimizer = optim.Adam(
            list(self.load_encoder_.parameters())
            + list(self.weather_encoder_.parameters()),
            lr=learning_rate,
        )
...
class GraphMultiModalClustering(BaseMultiModalClusteringMethod, LoggerMixin):
    ...
    def fit(
        ...
        if X_secondary is None:
            raise ValueError(
                "Graph multi-modal clustering requires secondary data (weather)"
            )
        ...
        if self.fusion_type == "concatenate":
            ...
            weight_weather = 0.4
            self.fused_graph_ = (
                weight_load * load_graph + weight_weather * weather_graph
            )
        else:
    ...
    def predict(
        ...
        distances = np.linalg.norm(
            embeddings[:, np.newaxis] - self.cluster_centers_[np.newaxis, :], axis=2
        )
...
class AttentionFusionEncoder(nn.Module):
    ...
    def forward(self, load_data, weather_data):
        ...
        if self.attention_type == "cross":
            ...
            attended_load, attended_weather = self.co_attention(
                load_features, weather_features
            )
            fused_features = torch.cat([attended_load, attended_weather], dim=1)
