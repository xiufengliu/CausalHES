    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            if epoch % 20 == 0:
                ...
                self.log_info(
                    f"Epoch {epoch}/{epochs} - Total: {avg_loss:.4f}, "
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (batch_x,) in enumerate(dataloader):
                ...
                z_perm = self._permute_dims(
                    z
                )  # Permute dimensions to break correlations
            ...
            if epoch % 20 == 0:
                ...
                self.log_info(
                    f"Epoch {epoch}/{epochs} - VAE Loss: {avg_vae_loss:.4f}, "
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (batch_load, batch_weather) in enumerate(dataloader):
                ...
                # Forward pass
                recon_load, recon_weather, mu, logvar = self.mvae_model_(
                    batch_load, batch_weather
                )

                # Compute losses
                recon_loss_load = F.mse_loss(recon_load, batch_load, reduction="sum")
                recon_loss_weather = F.mse_loss(
                    recon_weather, batch_weather, reduction="sum"
                )
                kld_loss = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
