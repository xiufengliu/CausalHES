...
class TwoStageKMeans(BaseClusteringMethod, LoggerMixin):
    ...
    def fit(self, X: np.ndarray, y: Optional[np.ndarray] = None) -> "TwoStageKMeans":
        ...
        """
        self.log_info(
            f"Fitting two-stage K-means with {self.n_clusters} total clusters"
        )
        ...
        self.consumption_kmeans = KMeans(
            n_clusters=self.k_consumption, random_state=self.random_state, n_init="auto"
        )
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        for consumption_cluster in range(self.k_consumption):
            ...
            if consumption_cluster in self.peaktime_kmeans:
                peak_features = self._extract_peak_features(cluster_data)
                peaktime_labels = self.peaktime_kmeans[consumption_cluster].predict(
                    peak_features
                )
...
class IntegralKMeans(BaseClusteringMethod):
    ...
    def __init__(
        self, n_clusters: int, dx: float = 0.25, random_state: Optional[int] = None
    ):
        """
