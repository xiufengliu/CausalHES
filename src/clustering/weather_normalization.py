...
class LinearWeatherNormalization(BaseClusteringMethod, LoggerMixin):
    ...
    def fit(
        self, X: np.ndarray, X_weather: np.ndarray, y: Optional[np.ndarray] = None
...
class NonlinearWeatherNormalization(BaseClusteringMethod, LoggerMixin):
    ...
    def fit(
        ...
        self.weather_model_ = WeatherPredictionNetwork(
            input_dim=n_weather_features, output_dim=1, hidden_dims=self.hidden_dims
        ).to(self.device)
...
class CDDHDDWeatherNormalization(BaseClusteringMethod, LoggerMixin):
    ...
    def fit(
        ...
        base_daily_load = (
            daily_load - predicted_weather_load + np.mean(predicted_weather_load)
        )
