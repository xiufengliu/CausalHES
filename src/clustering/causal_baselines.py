...
class DoublyRobustClustering(BaseMultiModalClusteringMethod, LoggerMixin):
    ...
    def fit(
        ...
        X_secondary: Optional[np.ndarray] = None,
        ...
        if X_secondary is None:
            raise ValueError(
                "Doubly robust clustering requires secondary data (weather)"
            )
        ...
        if self.propensity_model == "linear":
            self.propensity_estimator_ = self._fit_linear_propensity(
                X_primary, weather_treatment
            )
        else:
            self.propensity_estimator_ = self._fit_neural_propensity(
                X_primary, weather_treatment
            )
        ...
        if self.outcome_model == "linear":
            self.outcome_estimator_ = self._fit_linear_outcome(
                X_primary, weather_treatment
            )
        else:
            self.outcome_estimator_ = self._fit_neural_outcome(
                X_primary, weather_treatment
            )
    ...
    def predict(
        self, X_primary: np.ndarray, X_secondary: Optional[np.ndarray] = None, **kwargs
    ) -> np.ndarray:
    ...
    def _compute_dr_adjustment(self, X: np.ndarray, T: np.ndarray) -> np.ndarray:
        ...
        for t in range(n_timesteps):
            ...
            if self.propensity_model == "linear":
                ...
                with torch.no_grad():
                    prop_scores = (
                        torch.sigmoid(self.propensity_estimator_(X_tensor)[:, t])
                        .cpu()
                        .numpy()
                    )
...
class InstrumentalVariableClustering(BaseMultiModalClusteringMethod, LoggerMixin):
    ...
    def fit(
        ...
        if X_secondary is None:
            raise ValueError(
                "IV clustering requires secondary data (weather as instrument)"
            )
        ...
        for t in range(X_primary.shape[1]):
            ...
            if t > 0:
                instrument = X_secondary[
                    :, t - 1 : t + 1
                ]  # Current and previous weather
            else:
            ...
            if abs(correlation) < self.iv_strength_threshold:
                self.log_warning(
                    f"Weak instrument at timestep {t}, correlation: {correlation:.3f}"
                )
...
class DomainAdaptationClustering(BaseMultiModalClusteringMethod, LoggerMixin):
    ...
    def fit(
        ...
        if X_secondary is None:
            raise ValueError(
                "Domain adaptation clustering requires secondary data (weather)"
            )
        ...
        # Create models
        self.feature_extractor_ = FeatureExtractor(input_dim, self.embedding_dim).to(
            self.device
        )
        self.domain_classifier_ = DomainClassifier(self.embedding_dim, n_domains).to(
            self.device
        )
        self.cluster_classifier_ = ClusterClassifier(
            self.embedding_dim, self.n_clusters
        ).to(self.device)

        # Optimizers
        feature_optimizer = optim.Adam(
            self.feature_extractor_.parameters(), lr=learning_rate
        )
        domain_optimizer = optim.Adam(
            self.domain_classifier_.parameters(), lr=learning_rate
        )
        cluster_optimizer = optim.Adam(
            self.cluster_classifier_.parameters(), lr=learning_rate
        )
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (batch_x, batch_domain) in enumerate(dataloader):
                ...
                domain_adversarial_loss = -F.cross_entropy(
                    domain_pred_for_features, batch_domain
                )
                ...
                cluster_loss = -torch.mean(
                    torch.sum(cluster_pred * torch.log(cluster_pred + 1e-8), dim=1)
                )

                # Total feature loss
                feature_loss = (
                    self.domain_lambda * domain_adversarial_loss + cluster_loss
                )
            ...
            if epoch % 20 == 0:
                ...
                self.log_info(
                    f"Epoch {epoch}/{epochs} - Feature: {avg_feature_loss:.4f}, "
    ...
    def get_embeddings(
        self, X_primary: np.ndarray, X_secondary: np.ndarray
    ) -> np.ndarray:
    ...
    def _create_domain_labels(self, weather_data: np.ndarray) -> np.ndarray:
        ...
        domain_labels[(mean_temp > terciles[0]) & (mean_temp <= terciles[1])] = (
            1  # Medium
        )
...
class DomainClassifier(nn.Module):
    ...
    def __init__(self, input_dim: int, n_domains: int):
        super().__init__()
        self.network = nn.Sequential(
            nn.Linear(input_dim, 32), nn.ReLU(), nn.Linear(32, n_domains)
        )
...
class ClusterClassifier(nn.Module):
    ...
    def __init__(self, input_dim: int, n_clusters: int):
        super().__init__()
        self.network = nn.Sequential(
            nn.Linear(input_dim, 32), nn.ReLU(), nn.Linear(32, n_clusters)
        )
