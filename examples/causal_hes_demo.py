...
def generate_demo_data():
    ...
    for i in range(n_samples):
        ...
        temp = 20 + 10 * np.sin(2 * np.pi * np.arange(n_timesteps) / 24) + np.random.randn(n_timesteps)
        humidity = 50 + 20 * np.cos(2 * np.pi * np.arange(n_timesteps) / 24) + np.random.randn(n_timesteps)
...
def run_causal_hes_demo():
    ...
    try:
        ...
        print(f"Note: CausalHES model initialization needs PyTorch conversion completion: {e}")
    ...
    visualize_clustering_results(
        load_data, weather_data, true_labels, predicted_labels, 
...
def visualize_source_separation(separation_results, n_examples=5, save_dir=None):
    ...
    for i, idx in enumerate(indices):
        # Original load
        axes[i, 0].plot(separation_results['original_load'][idx, :, 0], 'b-', linewidth=2)
        ...
        # Weather effect
        axes[i, 2].plot(separation_results['weather_effect'][idx, :, 0], 'r-', linewidth=2)
        axes[i, 2].set_title('Weather Effect')
        axes[i, 2].set_ylabel('Load')
        axes[i, 2].grid(True)
        
        # Reconstruction comparison
        axes[i, 3].plot(separation_results['original_load'][idx, :, 0], 'b-', 
                       label='Original', linewidth=2)
        axes[i, 3].plot(separation_results['reconstructed_total'][idx, :, 0], 'r--', 
...
def visualize_clustering_results(load_data, weather_data, true_labels, predicted_labels, save_dir=None):
    """Visualize clustering results."""
    ...
    axes[0, 2].bar(x + width/2, pred_counts[:len(x)], width, label='Predicted', alpha=0.7)
    ...
    for i, label in enumerate(unique_pred[:3]):  # Show first 3 clusters
        ...
        for idx in sample_indices:
            axes[1, 1].plot(load_data[idx, :, 0], alpha=0.6, 
                           color=plt.cm.tab10(i), linewidth=1)
        
        # Plot cluster center
        mean_profile = np.mean(load_data[mask], axis=0)
        axes[1, 1].plot(mean_profile[:, 0], color=plt.cm.tab10(i), 
                       linewidth=3, label=f'Cluster {label}')
    ...
    scatter = axes[1, 2].scatter(weather_flat[indices], load_flat[indices], 
...
def print_training_summary(summary):
    ...
    print(f"  Loss weights: Recon={config['reconstruction_weight']}, "
